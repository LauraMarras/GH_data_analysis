...
def my_classifier(method='single_channel-all_bands', feature='envelope', window='feedback', window_length=1, classify='accuracy', participants=[], delay=0, repetitions='all'):
   if delay != 0:
        dela='_delay_' + str(delay)
   else:
        dela='_no delay' 
   data_path = 'C:/Users/laura/OneDrive/Documenti/Internship/Data_Analysis/Data/PreprocessedData_rereferencing/{}/'.format(reref + '/' + window + '-' + str(window_length) + '_' + classify + dela)
...
#### Classifier based on all bands power from one channel
   if method == 'single_channel-all_bands':
      ...
      # Load data   
         features = np.load(data_path + participant + '_rep_' + repetitions + '_bands_{}.npy'.format(feature))
         labels = np.load(data_path + participant + '_' + classify + '_rep_' + repetitions + '_labels.npy')
      ...
      # Save Figure
         ...
         plt.savefig(out_path + participant + '_' + method + '_rep_' + repetitions + '_' + reref, dpi=300)
...
#### Classifier based on single band power from all channels
   ...
   # Save Figures
      if not os.path.exists(out_path):
         os.makedirs(out_path)
      plt.savefig(out_path + method + '_rep_' + repetitions + '_' + reref, dpi=300)
...
if __name__=="__main__":
   ...
   for rep in repetitions:
      #Preprocessing_classifier.preprocess_data(reref=reref, feature=feature, window=window, window_length=window_length, delay=delay, subtract_delay=subtract_delay, classify=classify, participants=participants, repetitions=rep)
      for method in ['single_channel-all_bands', 'single_band-all_channels']: #'single_channel-all_bands', 'single_band-all_channels', 
         my_classifier(method=method, feature=feature, window=window, window_length=window_length, classify=classify, participants=participants, delay=delay, repetitions=rep)